...
cv_status condition_variable::wait_for(unique_lock<mutex>& lock,
    ...
    tpf max_time = tpi::max();
    system_clock::time_point sys_now = system_clock::now();
    steady_clock::time_point std_now = steady_clock::now();
    if (max_time - timeout_duration > sys_now) {
      do_timed_wait(lock, sys_now + ceil<nanoseconds>(timeout_duration));
    } else {
      do_timed_wait(lock, tpi::max());
    }
    return steady_clock::now() - std_now < timeout_duration ?
      cv_status::no_timeout : cv_status::timeout;
}
