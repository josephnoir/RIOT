...
/**
 * Available modes from each mode:
...
enum stm32l4_system_sleepmode {
    /**
     * Shutdown, manual section 5.3.10
     */
    SYSTEM_SLEEPMODE_SHUTDOWN,
    /**
     * Standby, manual section 5.3.9
     */
    SYSTEM_SLEEPMODE_STANDBY,
    /**
     * Stop 2, manual section 5.3.8
     */
    SYSTEM_SLEEPMODE_STOP2,
    /**
     * Stop 1, manual section 5.3.6
     */
    SYSTEM_SLEEPMODE_STOP1,
    /**
     * Stop 0, manual section 5.3.6
     */
    SYSTEM_SLEEPMODE_STOP0,
    /**
     * Low-power sleep, manual section 5.3.5
     */
    // SYSTEM_SLEEPMODE_LP_SLEEP,
    /**
     * Low-power run, manual section 5.3.2
     */
    // SYSTEM_SLEEPMODE_LOW_POWER_RUN,
    /**
     * Sleep, manual section 5.3.4
     */
 ...
 * others... /KS */
    ...
    switch (mode) {
        ...
        case SYSTEM_SLEEPMODE_STANDBY:
            ...
            RCC->BDCR |= RCC_BDCR_LSEON;
            /* LPMS = “011” in PWR_CR1 */
            PWR->CR1 &= ~(PWR_CR1_LPMS_SHUTDOWN); // clear the 0 bit
        ...
        case SYSTEM_SLEEPMODE_STOP2:
             ...
             * continues.
             */
            EXTI->PR &= ~(EXTI_PR1_PIF0);
            EXTI->PR &= ~(EXTI_PR1_PIF1);
            EXTI->PR &= ~(EXTI_PR1_PIF2);
            EXTI->PR &= ~(EXTI_PR1_PIF3);
            EXTI->PR &= ~(EXTI_PR1_PIF4);
            EXTI->PR &= ~(EXTI_PR1_PIF5);
            EXTI->PR &= ~(EXTI_PR1_PIF6);
            EXTI->PR &= ~(EXTI_PR1_PIF7);
            EXTI->PR &= ~(EXTI_PR1_PIF8);
            EXTI->PR &= ~(EXTI_PR1_PIF9);
            EXTI->PR &= ~(EXTI_PR1_PIF10);
            EXTI->PR &= ~(EXTI_PR1_PIF11);
            EXTI->PR &= ~(EXTI_PR1_PIF12);
            EXTI->PR &= ~(EXTI_PR1_PIF13);
            EXTI->PR &= ~(EXTI_PR1_PIF14);
            EXTI->PR &= ~(EXTI_PR1_PIF15);
            EXTI->PR &= ~(EXTI_PR1_PIF16);
            EXTI->PR &= ~(EXTI_PR1_PIF18);
            EXTI->PR &= ~(EXTI_PR1_PIF19);
            EXTI->PR &= ~(EXTI_PR1_PIF20);
            EXTI->PR &= ~(EXTI_PR1_PIF21);
            EXTI->PR &= ~(EXTI_PR1_PIF22);
            /* optionally enable RTC, LSI & LSE */
            RCC->BDCR |= RCC_BDCR_RTCEN;
            RCC->CSR |= RCC_CSR_LSION;
            RCC->BDCR |= RCC_BDCR_LSEON;
            ...
            /* LPMS = “010” in PWR_CR1 */
            PWR->CR1 &= ~(PWR_CR1_LPMS_SHUTDOWN); // clear the left 0 bit
            PWR->CR1 |= PWR_CR1_LPMS_STOP2;
            PWR->CR1 &= ~(PWR_CR1_LPMS_STOP1); // clear the right 0 bit
            deep = 1;
            break;
        case SYSTEM_SLEEPMODE_STOP1:
            /**
             * NOTE: To enter Stop 1 mode, all EXTI Line pending bits (in
             * Pending register 1 (EXTI_PR1)), and the peripheral flags
             * generating wakeup interrupts must be cleared. Otherwise, the
             * Stop 1 mode entry procedure is ignored and program execution
             * continues.
             */
            EXTI->PR &= ~(EXTI_PR1_PIF0);
            EXTI->PR &= ~(EXTI_PR1_PIF1);
             ...
             * wakeup time and regulator wakeup time from Low-power mode +
             * Flash wakeup time from Stop 1 mode.
             */
            /* LPMS = “001” in PWR_CR1 */
            PWR->CR1 &= ~(PWR_CR1_LPMS_SHUTDOWN); // clear the left 0 bit
            PWR->CR1 &= ~(PWR_CR1_LPMS_STOP2); // clear the middle 0 bit
            PWR->CR1 |= PWR_CR1_LPMS_STOP1;
            deep = 1;
            break;
        case SYSTEM_SLEEPMODE_STOP0:
            /**
             * All clocks in the VCORE domain are stopped; the PLL, the MSI,
             * the HSI16 and the HSE oscillators are disabled. Some
             * peripherals with the wakeup capability (I2Cx (x=1,2,3), U(S)
             * ARTx(x=1,2...5) and LPUART) can switch on the HSI16 to receive a
             * frame, and switch off the HSI16 after receiving the frame if it
             * is not a wakeup frame. In this case, the HSI16 clock is
             * propagated only to the peripheral requesting it.
             * SRAM1, SRAM2 and register contents are preserved. The BOR is
             * always available in Stop 0 mode. The consumption is increased
             * when thresholds higher than VBOR0 are used.
             */
            ...
            break;
        /*
        case SYSTEM_SLEEPMODE_LP_SLEEP:
            ...
            deep = 0;
            break;
        case SYSTEM_SLEEPMODE_LOW_POWER_RUN:
            ...
            // 1. Force the regulator in main mode by clearing the LPR bit
            //    in the PWR_CR1 register.
